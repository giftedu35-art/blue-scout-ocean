...
function imageData(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      resolve({ data, mimeType: (head.match(/data:(.*?);/) || [])[1] || 'image/jpeg' });
    };
...
async function analyzePhotoWithGemini(file) {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    if (!data.pending) { frame.remove(); form.remove(); if (!data.ok) throw new Error(data.error || 'AI 분석에 실패했어요.'); return showAiResult(data.result); }
  }
